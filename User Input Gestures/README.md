...
GestureCap is a standalone live gesture-control runtime. It does not depend on
...
```bash
gesturecap enroll delay_hold --model models/gestures.json --seconds 2 --show
gesturecap enroll filter_grab --model models/gestures.json --seconds 2 --show
```
...
```bash
gesturecap run --model models/gestures.json --host 127.0.0.1 --port 8000 --split-axes
```

Lowest-latency state settings are the defaults:

```bash
gesturecap run --enter-frames 1 --exit-frames 1 --switch-frames 1
```

If the active gesture flickers, keep entry immediate and only relax release:

```bash
gesturecap run --enter-frames 1 --exit-frames 2
```
...
```text
/gesturecap/state/active delay_hold 0.92
/gesturecap/state/event enter delay_hold 0.92
/gesturecap/gesture/delay_hold/active 1
/gesturecap/gesture/delay_hold/confidence 0.92
```

Continuous landmark vectors:

```text
/gesturecap/hand/right/index_mcp 0.42 0.71 -0.18
```

Axis-specific messages for Max dropdown routing with `--split-axes`:

```text
/gesturecap/hand/right/index_mcp/x 0.42
/gesturecap/hand/right/index_mcp/y 0.71
/gesturecap/hand/right/index_mcp/z -0.18
```
...
```bash
gesturecap inspect --model models/gestures.json
gesturecap remove delay_hold --model models/gestures.json
```
