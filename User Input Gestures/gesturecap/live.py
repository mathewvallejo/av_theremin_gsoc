    ...
    with _open_capture(cv2, camera, width, height) as capture:
        ...
        try:
            while True:
                ...
                client.send_message("/gesturecap/hand/visible", 1)
                _send_landmarks(client, hand_key, landmarks, osc_cfg)
                _send_prediction(client, prediction, state, osc_cfg)
                client.send_message("/gesturecap/frame", [frame_index, now_ms])
...
def _send_landmarks(
    ...
    for index, (x, y, z) in enumerate(landmarks):
        ...
        if osc_cfg.send_landmark_vectors:
            client.send_message(f"/gesturecap/hand/{hand_key}/{name}", [x, y, z])
        if osc_cfg.split_axis_messages:
            client.send_message(f"/gesturecap/hand/{hand_key}/{name}/x", x)
            client.send_message(f"/gesturecap/hand/{hand_key}/{name}/y", y)
            client.send_message(f"/gesturecap/hand/{hand_key}/{name}/z", z)
...
def _send_prediction(client: Any, prediction: Any, state: Any, osc_cfg: OscConfig) -> None:
    if state.event == "switch" and state.previous_label:
        client.send_message(f"/gesturecap/gesture/{state.previous_label}/active", 0)

    if prediction.accepted and state.active_label:
        label = state.active_label
        client.send_message("/gesturecap/state/active", [label, prediction.confidence])
        client.send_message(f"/gesturecap/gesture/{label}/active", 1)
        client.send_message(f"/gesturecap/gesture/{label}/confidence", prediction.confidence)
    elif osc_cfg.send_unknown_predictions:
        client.send_message("/gesturecap/state/active", ["unknown", 0.0])

    if state.event in {"enter", "switch", "exit"}:
        label = state.active_label or state.previous_label or "none"
        client.send_message("/gesturecap/state/event", [state.event, label, prediction.confidence])
        if state.event == "exit" and state.previous_label:
            client.send_message(f"/gesturecap/gesture/{state.previous_label}/active", 0)


def _send_no_hand(client: Any, now_ms: int, frame_index: int) -> None:
    client.send_message("/gesturecap/hand/visible", 0)
    client.send_message("/gesturecap/frame", [frame_index, now_ms])
