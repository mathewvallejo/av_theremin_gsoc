...
def parse_args():
    ...
    parser.add_argument("--flush-every", type=int, default=300, help="Flush CSV rows every N frames.")
...
def landmark_fieldnames():
    fields = ["video", "frame", "timestamp_ms", "num_hands"]
    for hand_i in range(2):
        fields.extend([f"hand{hand_i}_label", f"hand{hand_i}_score"])
        fields.extend([f"hand{hand_i}_{j}" for j in range(63)])
    return fields


def handedness_for_index(result, hand_i):
    if not result.handedness or hand_i >= len(result.handedness) or not result.handedness[hand_i]:
        return "", np.nan

    category = result.handedness[hand_i][0]
    return category.category_name.lower(), float(category.score)
...
def extract_landmarks_for_video(
    ...
    with partial_csv.open("w", newline="") as f, vision.HandLandmarker.create_from_options(landmarker_options) as landmarker:
        writer = csv.DictWriter(f, fieldnames=landmark_fieldnames())
        writer.writeheader()
        ...
        while True:
            ...
            for hand_i in range(2):
                label, score = handedness_for_index(result, hand_i)
                row[f"hand{hand_i}_label"] = label
                row[f"hand{hand_i}_score"] = score
            ...
            writer.writerow(row)
            frame_idx += 1
            if flush_every > 0 and frame_idx % flush_every == 0:
                f.flush()
