...
with vision.HandLandmarker.create_from_options(options) as landmarker:
    while True:
        ...
        row = {"video": str(args.video), "frame": frame_idx, "timestamp_ms": timestamp_ms, "num_hands": len(result.hand_landmarks)}

        for hand_i in range(2):
            if result.handedness and hand_i < len(result.handedness) and result.handedness[hand_i]:
                category = result.handedness[hand_i][0]
                row[f"hand{hand_i}_label"] = category.category_name.lower()
                row[f"hand{hand_i}_score"] = float(category.score)
            else:
                row[f"hand{hand_i}_label"] = ""
                row[f"hand{hand_i}_score"] = np.nan

            if hand_i < len(result.hand_landmarks):
                pts = np.array([[lm.x, lm.y, lm.z] for lm in result.hand_landmarks[hand_i]], dtype=np.float32).reshape(-1)
            else:
