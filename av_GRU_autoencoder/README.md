...
# Cleaning and Re-running Experiments

During development it is often necessary to regenerate windows, retrain models, or test multiple configurations.

Before re-running a stage, remove the artifacts generated by that stage to avoid mixing results from different experiments.

## Rebuild Window Data

Deletes all generated Stage 2 windows and manifests:

```bash
rm -rf <project_root>/Window_Data
```
...
```bash
python build_windows.py \
  --input-dir "<project_root>/Feature_Data" \
  --output-dir "<project_root>/Window_Data" \
  --sequence-length 60 \
  --hop-length 30
```
...
Deletes all training outputs:

```bash
rm -rf <project_root>/Model_Outputs
```
...
## Recompute Embeddings Only

Delete:

```bash
rm -rf <project_root>/Model_Outputs/embeddings
```
...
## Recompute Clustering Only

Delete:

```bash
rm -rf <project_root>/Model_Outputs/clustering
```

Then:

```bash
python cluster.py --config configs/small_test.yaml --method hdbscan
```

or:

```bash
python cluster.py --config configs/small_test.yaml --method kmeans --k 8
```
...
```bash
rm -rf <project_root>/Model_Outputs/reports
```
...
## Rebuild Runtime Export

Delete:

```bash
rm -rf <project_root>/Model_Outputs/export_for_runtime
```
