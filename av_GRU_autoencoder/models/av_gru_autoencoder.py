...
class AVGRUAutoencoder(nn.Module):
    ...
    Pooling strategy: mean over all encoder time steps, then LayerNorm.
    The runtime encoder (Stage 3) must reproduce this exactly.
    TorchScript export via export_for_runtime.py is the safest way to
    guarantee consistency — it exports the actual encode() method.
    """
    ...
    def encode(self, motion_seq):
        """Encode a motion sequence to a latent vector.

        Uses mean pooling over all time steps followed by LayerNorm + Linear.
        This exact pooling strategy is exported via TorchScript for Stage 3.
        """
